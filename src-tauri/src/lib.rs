...
async fn show_highlight_popup(app: AppHandle, data: PopupData) -> Result<String, String> {
    ...
    let saved_position = match app.store("window-state.json") {
        Ok(store) => {
            println!("[弹窗] 成功打开store");
            let val = store.get("popup_position");
            println!("[弹窗] 读取 popup_position: {:?}", val);
            val.and_then(|v| {
                let x = v.get("x").and_then(|v| v.as_i64()).map(|v| v as i32);
                let y = v.get("y").and_then(|v| v.as_i64()).map(|v| v as i32);
                println!("[弹窗] 解析位置: x={:?}, y={:?}", x, y);
    ...
    let (x, y) = if let Some((sx, sy)) = saved_position {
        ...
        if let Some(m) = monitor {
            ...
            let x = (screen_width - default_width) / 2.0;
            let y = (screen_height - default_height) / 2.0;
            println!("[弹窗] 使用默认位置: ({}, {})", x as i32, y as i32);
            (x as i32, y as i32)
        } else {
            (100, 100)
        }
    ...
    .min_inner_size(280.0, 150.0)
    .max_inner_size(280.0, 2000.0)
    .always_on_top(true)
    .decorations(true)
    .transparent(false)
    .skip_taskbar(false)
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let payload = serde_json::json!({
            "popup_id": popup_id_clone,
            "data": data_clone
        });
        let _ = window_clone.emit("popup-initialized", &payload);
        println!("[Rust] 弹窗数据已发送: {}", popup_id_clone);
    });
...
async fn save_popup_state(app: AppHandle, x: i32, y: i32) -> Result<(), String> {
    println!("[弹窗] save_popup_state 被调用: x={}, y={}", x, y);
    
    let store = app.store("window-state.json")
        .map_err(|e| format!("获取存储失败: {}", e))?;
    
    let position = serde_json::json!({ "x": x, "y": y });
    println!("[弹窗] 设置 popup_position: {:?}", position);
    store.set("popup_position", position.clone());
    
    match store.save() {
        Ok(_) => {
            println!("[弹窗] 保存成功");
            // 验证保存
            if let Some(v) = store.get("popup_position") {
                println!("[弹窗] 验证: 读取到 {:?}", v);
            }
        }
        Err(e) => {
...
#[tauri::command]
async fn open_image_viewer(app: AppHandle, image_url: String, width: Option<u32>, height: Option<u32>) -> Result<String, String> {
    use std::sync::atomic::{AtomicU32, Ordering};
    static IMAGE_COUNTER: AtomicU32 = AtomicU32::new(0);
    
    let window_id = format!("image-viewer-{}", IMAGE_COUNTER.fetch_add(1, Ordering::SeqCst));
    
    let win_width = width.unwrap_or(800) as f64;
    let win_height = height.unwrap_or(600) as f64;
    
    let max_width = 1400.0;
    let max_height = 900.0;
    let final_width = win_width.min(max_width).max(200.0);
    let final_height = win_height.min(max_height).max(150.0);
    ...
    let (x, y) = if let Some(m) = monitor {
        ...
        let pos_x = (screen_width - final_width) / 2.0;
        let pos_y = (screen_height - final_height) / 2.0;
    ...
    let window = tauri::WebviewWindowBuilder::new(
        &app,
        &window_id,
        tauri::WebviewUrl::App("image.html".into()),
    )
    .title("图片查看")
    ...
    tauri::async_runtime::spawn(async move {
        std::thread::sleep(std::time::Duration::from_millis(200));
        let _ = window_clone.emit("image-data", serde_json::json!({
            "url": image_url_clone
        }));
    });
        ...
        .setup(|app| {
            #[cfg(desktop)]
            {
                ...
                if let Some(window) = app.get_webview_window("main") {
                    ...
                    let saved_state: Option<WindowState> = if let Ok(store) = app.store("window-state.json") {
                        if let Some(value) = store.get("main_window") {
                    ...
                    if let Some(state) = saved_state {
                        ...
                        std::thread::spawn(move || {
                            ...
                            let _ = window_clone.set_position(tauri::PhysicalPosition::new(state.x, state.y));
                            let _ = window_clone.set_size(tauri::PhysicalSize::new(state.width, state.height));
                        });
                    }
                }
            }
